...
This project details the integration of machine learning into the micro:bit Python Editor V3 project. The end goal of this project is to enable a machine learning model trained in browser to run on the micro:bit device and for it to be accessible through code made in the micro:bit Python Editor.

The machine learning model trained in this project functions the same as the one used in the [ML-Machine](https://ml-machine.org/) project, whose source code can be found at https://github.com/microbit-foundation/cctd-ml-machine. The user is expected to record training data on the aformentioned website before the use of this project.
The produced dataset file is then used by the micro:bit Python Editor to train a model in browser and make methods similar to the ones of the accelerometer module available to the user.
The current state of the project uses a Python module added to the initial project to simulate the machine learning model running on the device by selecting a random class name from the included text file on the device. These names are based on the model trained in browser and the names chosen when the data was first recorded.

## Additional repositories needed for the development of this project
